...
describe("Filter Spec", () => {
    beforeEach(() => {
      ...
      it("Rename Filter", () => {
        ...
        cy.get("#design-name-textfield").type("Changed Filter Name with cypress");
        ...
        cy.wait("@filterSave").then(() => {
          ...
          cy.get("#MUIDataTableBodyRow-filters-0 p").contains("Changed Filter Name with cypress");
        })
      })

      it("Search a filter", () => {
        cy.get("[data-cy='filter-drawer']").click();
        cy.get('[data-test-id="Search"]').type("Changed Filter Name with cypress");
        cy.intercept("/api/filter*").as("filterSearch")
        cy.wait("@filterSearch")
        cy.get("#MUIDataTableBodyRow-filters-0").should("be.visible").contains("Changed Filter Name with cypress");
      });
