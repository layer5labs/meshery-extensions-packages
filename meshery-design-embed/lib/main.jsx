...
const MesheryDesignEmbed = ({
  ...
  useEffect(() => {
    ...
    const customizeToolbar = () => {
      ...
      // Inject CSS to hide original toolbar and show custom one
      const styleId = toolbar-`${embedId}`;
      ...
      // Create custom toolbar if it doesn't exist
      if (!embedContainer.querySelector('.custom-meshery-toolbar')) {
        const customToolbar = document.createElement('div');
        customToolbar.className = 'custom-meshery-toolbar';
        customToolbar.innerHTML = `
          <a href="https://cloud.layer5.io/catalog/content/design/${extractUUID(embedId)}" target="_blank" rel="noopener noreferrer">
        ...
        `;
        embedContainer.appendChild(customToolbar);
      }
    };

    // Wait for embed to load then customize
    const timer = setInterval(() => {
      const embedContainer = document.getElementById(embedId);
      if (embedContainer && (
        embedContainer.querySelector('.toolbar') || 
        embedContainer.querySelector('.water-mark')
      )) {
        customizeToolbar();
        clearInterval(timer);
      }
    ...
    return () => {
      clearInterval(timer);
      const styleEl = document.getElementById(toolbar-`${embedId}`);
